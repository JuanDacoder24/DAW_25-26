...
app.put('/api/tasks/:id', async (req, res) => {
    try {
        ...
        await connection.execute('UPDATE tasks SET title = ?, description = ?, completed = ? WHERE id = ?', [title, description, completed, req.params.id]);
        const [updatedTask] = await connection.execute('SELECT * FROM tasks WHERE id = ?', [req.params.id]);
        const updatedTaskArray = updatedTask;
